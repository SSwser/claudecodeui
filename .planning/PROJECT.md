...
|----------|-----------|---------|
| 核心价值定位为"组织管理" | 基于实际使用痛点,多项目/会话管理是最突出的问题 | — Pending |
| v1 包含 UI 组件库迁移 | 新功能需要现代化 UI 支撑,同步进行避免二次重构 | — Pending |
| 虚拟会话采用混合模式 | 默认透明降低认知负担,可视化管理满足高级用户需求 | — Pending |
| 移动端分阶段开发 | 先完成桌面端核心体验,再适配移动端 | — Pending |
| 桌面端支持双模式打包 | 满足不同用户场景:本地隐私 vs 云端同步 | — Pending |
