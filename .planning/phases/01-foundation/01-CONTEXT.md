...
- Name: [________]
- Type:
  o Logical workspace (shared filesystem) [DEFAULT]
  o Independent worktree (create new git worktree)
...
- `src/components/sidebar/view/Sidebar.tsx` - Navigation patterns
- `src/components/main-content/view/MainContent.tsx` - Layout container patterns
- `src/components/chat/view/ChatInterface.tsx` - Session rendering (will be embedded in panes)

### Existing Routes

- `src/App.tsx` - Route definitions
- Current entry point: `/` and `/session/:sessionId`
- Need to keep `/session/:sessionId` working while adding landing/home behavior

### Database Schema (Existing)

- `server/database/init.sql` - users, sessions, naming tables
- Phase 1 may extend persistence for logical workspaces or startup metadata if localStorage is insufficient
...
- Cross-device favorites sync (v2)
...
*Normalized into phase directory for planner compatibility*
