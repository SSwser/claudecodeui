...
- **Deep Amber** (`hsl(44, 100%, 45%)` / ~`#e5a700`): Warning accents, highlights — darker than brand red to prevent confusion
...
- **Warning Amber** (`hsl(44, 100%, 45%)` / ~`#e5a700`): Warnings, attention-needed states — deliberately darker/more saturated than brand red to maintain clear semantic separation
...
### Card Shadow Practice (Level 2 Ring)

All card-type containers (session cards, AI message bubbles, skeleton loaders) use the **Level 2 Ring** as
their baseline shadow. The ring replaces visible borders with a macOS-native double-ring containment effect:
...
**Stroke alpha varies by interaction state** — the ring effect stays constant, only the border stroke
intensity changes to signal state:
...
**Elements NOT using Level 2 Ring** (each has a purpose-built shadow system):
