...
<p align="center">
  <a href="https://chorus.dev">Chorus Cloud</a> · <a href="https://chorus.dev/docs">ドキュメント</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/siteboon/claudecodeui/issues">バグ報告</a> · <a href="CONTRIBUTING.md">コントリビュート</a>
</p>
...
- **プラグインシステム** - カスタムプラグインで Chorus を拡張 — 新しいタブ、バックエンドサービス、連携を追加できます。[自分で構築する →](https://github.com/chorus-dev/chorus-plugin-starter)
...
**[Chorus Cloud を始める](https://chorus.dev)**

### セルフホスト（オープンソース）

**npx** で今すぐ Chorus を試せます（**Node.js** v22+ が必要）：
...
より詳細な設定オプション、PM2、リモートサーバー設定などについては **[ドキュメントはこちら →](https://chorus.dev/docs)** を参照してください。

---

## どちらの選択肢が適していますか？

Chorus は、Chorus Cloud を支えるオープンソースの UI レイヤーです。自分のマシンにセルフホストすることも、フルマネージドのクラウド環境、チーム機能、より深い統合を備えた Chorus Cloud を使うこともできます。

|                                 | Chorus（セルフホスト）                                                      | Chorus Cloud                                                   |
| ------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
...
| **セットアップ**                | `npx @anthropic-ai/chorus`                                                  | セットアップ不要                                               |
| **マシンの稼働継続**            | はい                                                                        | いいえ                                                         |
...
> どちらの選択肢でも、AI のサブスクリプション（Claude、Cursor など）はご自身のものを使用します — Chorus が提供するのは環境であり、AI そのものではありません。
...
_Tools 設定画面 - 必要なものだけを有効にしてください_
...
Chorus にはプラグインシステムがあり、独自のフロントエンド UI と（必要に応じて）Node.js バックエンドを持つカスタムタブを追加できます。プラグインは **Settings > Plugins** から git リポジトリを直接指定してインストールするか、自作できます。

### 利用可能なプラグイン

| プラグイン                                                               | 説明                                                                                                               |
| ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| **[Project Stats](https://github.com/chorus-dev/chorus-plugin-starter)** | 現在のプロジェクトについて、ファイル数、コード行数、ファイル種別の内訳、最大ファイル、最近変更されたファイルを表示 |

### 自作する

**[Plugin Starter Template →](https://github.com/chorus-dev/chorus-plugin-starter)** — このリポジトリを fork して独自プラグインを作れます。フロントエンド描画、ライブコンテキスト更新、バックエンドサーバーへの RPC 通信を含む動作例が入っています。

**[プラグインのドキュメント →](https://chorus.dev/docs/plugin-overview)** — プラグイン API、manifest 形式、セキュリティモデルなどの完全ガイド。
...
Chorus と Chorus Cloud は、Claude Code の横に別物として存在するのではなく、Claude Code を拡張します — MCP サーバー、権限、設定、セッションは Claude Code がネイティブに使うものと完全に同一です。複製したり、別系統で管理したりしません。

- **すべてのセッションにアクセス** — Chorus は `~/.claude` フォルダのすべてのセッションを自動検出します。Remote Control は、Claude モバイルアプリで利用可能にするため、1つのアクティブセッションだけを公開します。
- **設定はあなたの設定** — Chorus で変更した MCP サーバー、ツール権限、プロジェクト構成は、Claude Code の設定に直接書き込まれて即座に反映され、その逆（Claude Code での変更が UI に反映）も同様です。
...
- **Chorus Cloud はクラウド上で稼働** — ノートパソコンを閉じてもエージェントは動き続けます。監視が要る端末も、スリープ防止も不要です。
...
はい。Chorus は環境を提供するものであり、AI は含まれません。Claude、Cursor、Codex、または Gemini のサブスクリプションはご自身でご用意ください。Chorus Cloud のホスティング環境はそれに加えて月額 $7 から提供されます。

</details>

<details>
<summary>Chorus をスマホで使えますか？</summary>

はい。セルフホストの場合は、自身のマシンでサーバーを起動し、ネットワーク内のブラウザで `[yourip]:port` を開いてください。Chorus Cloud を使う場合は、任意のデバイスからアクセスできます。VPN もポートフォワーディングも不要で、セットアップも不要です。ネイティブアプリも開発中です。
...
はい、セルフホストの場合です。Chorus は Claude Code がネイティブに使う `~/.claude` 設定を読み書きします。UI から追加した MCP サーバーは即座に Claude Code に反映され、その逆も同様です。
...
- **[ドキュメント](https://chorus.dev/docs)** — インストール、設定、機能、トラブルシューティング
...
- **[TaskMaster AI](https://github.com/eyaltoledano/claude-task-master)** _(オプション)_ - AI を活用したプロジェクト管理とタスク計画
