...
<p align="center">
  <a href="https://chorus.dev">Chorus Cloud</a> · <a href="https://chorus.dev/docs">文档</a> · <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/siteboon/claudecodeui/issues">Bug 报告</a> · <a href="CONTRIBUTING.md">贡献指南</a>
</p>
...
**[立即开始 Chorus Cloud](https://chorus.dev)**

### 自托管（开源）

启动 Chorus，只需一行 `npx`（需要 Node.js v22+）：
...
更多配置选项、PM2、远程服务器设置等，请参阅 **[文档 →](https://chorus.dev/docs)**
...
Chorus 配备插件系统，允许你添加带自定义前端 UI 和可选 Node.js 后端的选项卡。在 Settings > Plugins 中直接从 Git 仓库安装插件，或自行开发。
...
**[Plugin Starter Template →](https://github.com/chorus-dev/chorus-plugin-starter)** — Fork 该仓库以构建自己的插件。示例包括前端渲染、实时上下文更新和 RPC 通信。

**[插件文档 →](https://chorus.dev/docs/plugin-overview)** — 提供插件 API、清单格式、安全模型等完整指南。
...
Chorus 与 Chorus Cloud 是对 Claude Code 的扩展，而非旁观 — MCP 服务器、权限、设置、会话与 Claude Code 完全一致。

- **覆盖全部会话** — Chorus 会自动扫描 `~/.claude` 文件夹中的每个会话。Remote Control 只暴露当前活动的会话。
- **设置统一** — 在 Chorus 中修改的 MCP、工具权限等设置会立即写入 Claude Code。
- **支持更多 Agents** — Claude Code、Cursor CLI、Codex、Gemini CLI。
- **完整 UI** — 除了聊天界面，还包括文件浏览器、Git 集成、MCP 管理和 Shell 终端。
- **Chorus Cloud 保持运行于云端** — 关闭本地设备也不会中断代理运行，无需监控终端。
...
需要。Chorus 只提供环境。你仍需自行获取 Claude、Cursor、Codex 或 Gemini 订阅。Chorus Cloud 从 $7/月起提供托管环境。

</details>

<details>
<summary>能在手机上使用 Chorus 吗？</summary>

可以。自托管时，在你的设备上运行服务器，然后在网络中的任意浏览器打开 `[yourip]:port`。Chorus Cloud 可从任意设备访问，内置原生应用也在开发中。
...
会的。自托管模式下，Chorus 读取并写入 Claude Code 使用的 `~/.claude` 配置。通过 UI 添加的 MCP 服务器会立即在 Claude Code 中可见。
...
- **[文档](https://chorus.dev/docs)** — 安装、配置、功能与故障排除指南
