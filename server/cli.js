 ...
 * Chorus CLI
 *
 * Provides command-line utilities for managing Chorus
 *
 * Commands:
...
function showStatus() {
  ...
  console.log(
    `       SERVER_PORT: ${c.bright(process.env.SERVER_PORT || process.env.PORT || '3001')} ${c.dim(process.env.SERVER_PORT || process.env.PORT ? '' : '(default)')}`
  );
  console.log(
  ...
  console.log('\n' + c.dim('═'.repeat(60)));
  console.log(`\n${c.tip('[TIP]')} Hints:`);
  console.log(`      ${c.dim('>')} Use ${c.bright('chorus --port 8080')} to run on a custom port`);
  console.log(
    `      ${c.dim('>')} Use ${c.bright('chorus --database-path /path/to/db')} for custom database`
  );
  console.log(`      ${c.dim('>')} Run ${c.bright('chorus help')} for all options`);
  console.log(
    `      ${c.dim('>')} Access the UI at http://localhost:${process.env.SERVER_PORT || process.env.PORT || '3001'}\n`
  );
}
...
function showHelp() {
  console.log(`
╔═══════════════════════════════════════════════════════════════╗
║              Chorus - Command Line Tool               ║
╚═══════════════════════════════════════════════════════════════╝

Usage:
  chorus [command] [options]

Commands:
  start          Start the Chorus server (default)
  status         Show configuration and data locations
...
Examples:
  ...
  $ chorus --port 8080            # Start on port 8080
  $ chorus -p 3000                # Short form for port
  $ chorus start --port 4000      # Explicit start command
  $ chorus status                 # Show configuration
...
async function checkForUpdates(silent = false) {
  try {
    ...
    if (isNewerVersion(latestVersion, currentVersion)) {
      ...
      console.log(`         Run ${c.bright('chorus update')} to update\n`);
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log('   Run "chorus help" for usage information.\n');
      process.exit(1);
  }
}
