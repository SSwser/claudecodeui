 ...
 *
 * provider + session_id are stored UNINDEXED — used in JOINs but not as FTS5 MATCH targets.
...
export function runMigration(db) {
  // If the table already exists without provider/session_id (created by an earlier inline
  // migration), drop it so we can recreate with the correct schema. The FTS index is purely a
  // search cache; data loss is safe — searchService.reindexAll() rebuilds it from session_state.
  const existing = db
  ...
  if (
    existing &&
    existing.sql &&
    (!existing.sql.includes('session_id') || !existing.sql.includes('provider'))
  ) {
