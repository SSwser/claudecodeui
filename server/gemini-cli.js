...
async function spawnGemini(command, options = {}, ws) {
  const { sessionId, projectPath, cwd, toolsSettings, permissionMode, images, sessionSummary } =
    options;
  ...
  return new Promise((resolve, reject) => {
    ...
    const registerRuntime = (runtimeSessionId) => {
      ...
      try {
        ...
        console.warn(
          `[gemini-cli] Failed to register lifecycle state for ${runtimeSessionId}:`,
          error.message
        );
      }
    };
    ...
    geminiProcess.on('error', (error) => {
      ...
      const errorSessionId =
        typeof ws.getSessionId === 'function' ? ws.getSessionId() : finalSessionId;
      ws.send(
        createNormalizedMessage({
          kind: 'error',
