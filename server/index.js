...
app.post('/api/projects/:projectName/files/create', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(409)
        .json({ error: `${type === 'file' ? 'File' : 'Directory'} already exists` });
    } catch {
    ...
    try {
      ...
      if (provider === 'codex') {
        ...
        if (!sessionFilePath) {
          return res
            .status(404)
            .json({ error: 'Codex session file not found', sessionId: safeSessionId });
        }
...
const VITE_PORT = process.env.VITE_PORT || 4000;
...
async function listenWithReuse(serverInstance, port, host) {
    ...
    const handleError = async (error) => {
      ...
      if (error?.code === 'EADDRINUSE') {
        ...
        if (existingServer) {
          ...
          resolve({ reused: true, existingServer });
          return;
        }

        reject(
          new Error(
            `Port ${port} is already in use by another process. Set SERVER_PORT to a free port and restart.`
          )
        );
        return;
      }
...
async function startServer() {
  try {
    ...
    if (isProduction) {
      console.log(
        `${c.info('[INFO]')} To run in production mode, go to http://${DISPLAY_HOST}:${SERVER_PORT}`
      );
    }

    console.log(
      `${c.info('[INFO]')} To run in development mode with hot-module replacement, go to http://${DISPLAY_HOST}:${VITE_PORT}`
    );

    const listenResult = await listenWithReuse(server, SERVER_PORT, HOST);
