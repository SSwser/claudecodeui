...
/**
 * Return the git "common dir" (the .git directory of the main repo) for the
 * given project path by calling `git rev-parse --git-common-dir`.
 *
 * All worktrees of the same repository share the identical gitCommonDir, so
 * grouping projects by this value reliably detects multi-stream (worktree) sets
 * without depending on any directory-naming convention.
 *
 * Returns null if the directory is not inside a git repo or git is unavailable.
 */
async function getGitCommonDir(projectPath) {
  ...
  try {
    ...
    const resolved = path.resolve(projectPath, stdout.trim());
    return resolved;
  } catch {
    return null;
  }
 ...
 * Returns null if not a git repo, git is unavailable, or HEAD is detached.
 */
...
async function getProjects(progressCallback = null) {
  ...
  try {
    ...
    for (const entry of directories) {
      ...
      const project = {
        ...
        gitCommonDir: await getGitCommonDir(actualProjectDir),
        sessions: [],
        geminiSessions: [],
        sessionMeta: {
          hasMore: false,
  ...
  for (const [projectName, projectConfig] of Object.entries(config)) {
    if (!existingProjects.has(projectName) && projectConfig.manuallyAdded) {
      ...
      try {
        ...
        console.warn(
          `Could not load Gemini sessions for manual project ${projectName}:`,
          e.message
        );
      }
      applyCustomSessionNames(project.geminiSessions, 'gemini');
  ...
  // Second pass: propagate gitCommonDir from parent projects to worktrees whose
  // actual path could not be determined (no JSONL files → fallback decode corrupts
  // the path → git command fails → null).  Claude encodes each path character as a
  // single '-', so a project whose name starts with another project's name followed
  // by at least one '-' is guaranteed to be inside the parent's directory tree and
  // therefore shares the same git repo.
  const projectsWithGitDir = projects.filter((p) => p.gitCommonDir != null);
  for (const project of projects) {
    if (project.gitCommonDir != null) continue;
    const parent = projectsWithGitDir.find((p) => project.name.startsWith(p.name + '-'));
    if (parent) {
      project.gitCommonDir = parent.gitCommonDir;
    }
  }
...
async function searchConversations(query, limit = 50, onProjectResult = null, signal = null) {
  ...
  try {
    ...
    for (const projectEntry of projectDirs) {
      ...
      const displayName =
        config[projectName]?.displayName || (await generateDisplayName(projectName));
