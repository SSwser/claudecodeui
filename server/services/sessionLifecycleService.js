...
function normalizeProvider(provider) {
  const normalized = String(provider || DEFAULT_PROVIDER)
    .trim()
    .toLowerCase();
  if (!normalized) {
...
function upsertSessionState({
  ...
  ).run(normalizedSessionId, resolvedWorkspaceId, normalizedProvider, status, title, summary);
...
function registerProcess(optionsOrSessionId, pid, provider = DEFAULT_PROVIDER) {
  ...
  const runtimeType =
    normalizedOptions.runtimeType || (normalizedOptions.process ? 'child-process' : 'virtual');
