  ...
  if (sessions.length === 0) {
    return (
      <section className="rounded-large border border-border/70 bg-card/95 p-6 shadow-ring">
        ...
        <p className="rounded-medium border border-dashed border-border/70 px-4 py-6 text-sm tracking-body text-muted-foreground">
      ...
      <div className="space-y-3">
        {sessions.map((session) => (
          <div
            ...
            className="group w-full rounded-large border border-border/70 bg-surface-2 p-4 text-left shadow-subtle transition-opacity hover:opacity-60"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Star
                    className={`h-4 w-4 ${session.isFavorite ? 'fill-current text-brand' : 'text-muted-foreground'}`}
                  />
                </Button>
