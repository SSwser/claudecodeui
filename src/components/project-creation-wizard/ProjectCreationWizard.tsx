	...
	const handleCreate = useCallback(async () => {
		...
		try {
			...
			const project = await createWorkspaceRequest({
				...
				githubTokenId: formState.tokenMode === 'stored' ? formState.selectedGithubToken || undefined : undefined,
				newGithubToken: formState.tokenMode === 'new' ? formState.newGithubToken.trim() || undefined : undefined,
			})

			onProjectCreated?.(project)
			onClose()
		} catch (createError) {
			const errorMessage = createError instanceof Error ? createError.message : t('projectWizard.errors.failedToCreate')
