  ...
  return createPortal(
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="flex max-h-[80vh] w-full max-w-2xl flex-col rounded-large border border-border/70 bg-surface-2 text-foreground shadow-ring">
        ...
        {showNewFolderInput && (
          <div className="bg-brand/8 border-b border-border/70 px-4 py-3">
            <div className="flex items-center gap-2">
              <Input
                type="text"
                value={newFolderName}
            ...
            <div className="space-y-1">
              {parentPath && (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-large px-4 py-3 text-left transition-colors hover:bg-surface-3"
                >
              ...
              {visibleFolders.length === 0 ? (
                <div className="py-8 text-center text-muted-foreground">No subfolders found</div>
              ) : (
                visibleFolders.map((folder) => (
                  <div key={folder.path} className="flex items-center gap-2">
                    <button
                      onClick={() => loadFolders(folder.path)}
                      className="flex flex-1 items-center gap-3 rounded-large px-4 py-3 text-left transition-colors hover:bg-surface-3"
                    >
        ...
        <div className="border-t border-border/70">
          <div className="flex items-center gap-2 bg-surface-3/55 px-4 py-3">
            <span className="text-sm text-muted-foreground">Path:</span>
            <code className="flex-1 truncate font-mono text-sm text-foreground">{currentPath}</code>
          </div>
