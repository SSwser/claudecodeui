      ...
      <div className="relative flex gap-2">
        <div className="relative flex-1">
          <Input
            ...
            placeholder={
              workspaceType === 'worktree' ? '/path/to/new/worktree' : '/path/to/workspace'
            }
            className="w-full"
            disabled={disabled}
          />
          ...
          {showPathDropdown && pathSuggestions.length > 0 && (
            <div className="absolute z-10 mt-1 max-h-60 w-full overflow-y-auto rounded-large border border-border/70 bg-surface-2 shadow-ring">
              {pathSuggestions.map((suggestion) => (
                <button
                  key={suggestion.path}
                  onClick={() => handleSuggestionSelect(suggestion)}
                  className="w-full px-4 py-2 text-left text-sm transition-colors hover:bg-surface-3"
                >
