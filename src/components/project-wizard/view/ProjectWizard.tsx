...
export default function ProjectWizard({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setError(
        creationError instanceof Error ? creationError.message : t('wizard.errors.createFailed')
      );
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <>
          ...
          <div className="space-y-6 px-6 pb-2 pt-1">
            ...
            {step === 2 ? (
              <WizardStepDirectory
                ...
                // Browsers do not expose a native folder picker that returns a filesystem path we
                // can pass back to the server, so use the authenticated folder browser instead.
                onBrowseRequest={() => setShowFolderBrowser(true)}
              />
            ) : null}
              ...
              <div className="flex items-center gap-2">
                {step > 1 ? (
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      setStep((previousStep) => Math.max(previousStep - 1, 1) as WizardStep)
                    }
                    disabled={isSubmitting}
                  >
        ...
        <FolderBrowserModal
          ...
          onFolderSelected={(selectedPath) => handleFolderSelected(selectedPath)}
        />
      </>
    </Dialog>
  );
