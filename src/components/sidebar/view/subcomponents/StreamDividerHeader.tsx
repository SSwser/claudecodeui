...
export default function StreamDividerHeader({ name, onClick }: StreamDividerHeaderProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex h-6 w-full cursor-pointer items-center gap-[6px] py-0 pl-3 pr-[14px] transition-colors hover:bg-surface-2"
    >
      <span className="h-px w-5 flex-shrink-0 bg-border" />
